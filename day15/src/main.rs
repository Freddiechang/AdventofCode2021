...
fn update_neighbor(map: &Vec<Vec<u32>>, visited: &Vec<Vec<bool>>, cost: &mut Vec<Vec<u32>>, x: usize, y: usize) {
    let x_max = map.len() - 1;
    let y_max = map[0].len() - 1;
    let f = |x| x as i32;
    let neighbor: Vec<(i32, i32)> = vec![(f(x), f(y) - 1), (f(x) - 1, f(y)),
                (f(x) + 1, f(y)), (f(x), f(y) + 1)];
    let valid = neighbor.iter()
        .filter(|(i, j)| *i >= 0 && *i <= x_max as i32 && *j >= 0 &&
            *j <= y_max as i32 && visited[*i as usize][*j as usize] == true);
    for i in valid {
    ...
    for x in 0..map.len() {
        for y in 0..map[0].len() {
            ...
            for k in valid {
                ...
                if temp < cost[x][y] { cost[x][y] = temp; }
            }
            visited[x][y] = true;
...
fn part_1(content_lines: &Vec<&str>) -> u32 {
    let map: Vec<Vec<u32>> = content_lines.iter().map(|x| x.chars().map(|x| x.to_digit(10).unwrap()).collect()).collect();
    shortest_path(&map)
}
...
fn part_2(content_lines: &Vec<&str>) -> u32 {
    ...
    let f  = |x: &Vec<u32>, i: u32| -> Vec<u32> { x.iter().map(|x| (x + i) % 10 + (x + i) / 10 ).collect() };
    for i in 0..5 {
        for j in 0..5 {
            for k in 0..map.len() {

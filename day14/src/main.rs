...
fn insertion_step(count: HashMap<String, u64>, mappings: &HashMap<&str, &str>) -> HashMap<String, u64> {
    let mut new_count: HashMap<String, u64> = HashMap::new();
    let mut f = |x: String, v: u64| {
        if new_count.contains_key(&x[..]) { *new_count.get_mut(&x[..]).unwrap() += v; }
        else { new_count.insert(x, v); }
    };
...
fn solution(original_seq: &str, mappings: &HashMap<&str, &str>, steps: u64) -> u64 {
    ...
    for (k, v) in pairs.iter() {
        ...
        for i in elements.iter() {
            if count.contains_key(i) { *count.get_mut(i).unwrap() += *v as u64; }
            else { count.insert(*i, *v as u64); }
        }
    }
    for (k, v) in count.iter_mut() {
        if *k == first || *k == last { *v = (*v - 1) / 2 + 1; }
        else { *v /= 2; }
    }
    let mut max: u64 = 0;
